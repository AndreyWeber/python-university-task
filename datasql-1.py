...
class datasql(data):
    def read(self):
        ...
        data=curs.fetchall()
        for r in data: self.getRas().createService(r[0],self.getRas().getKindService(int(r[1])),r[2],self.getRas().getClient(int(r[3])),r[4],r[5])
        conn.close()
    def write(self):
        ...
        for b in self.getRas().getServiceList():
            curs.execute("insert into service(code,kindService,count,client,dateReception,dateReturn) values('%s','%s','%s','%s','%s','%s')"%(
                b.getCode(),str(b.getKindService().getCode()),b.getCount(),str(b.getClient().getCode()),str(b.getDateReception()),str(b.getDateReturn())))
        conn.commit()   
        conn.close()
...
dat2=dataxml(ras2,r'C:\python\source\repos\PythonApplication\oldfile.xml',r'C:\python\source\repos\PythonApplication\newfile.xml')
